      ...
      <div className="max-w-3xl mx-auto">
        <div className={`${styles.card} rounded-xl ${optimizationSettings.useShadowEffects ? 'shadow-xl' : styles.border} overflow-hidden`}>
            ...
            {activeTab === 'overview' && (
              <div className={`${optimizationSettings.useAnimations ? 'animate-fade-in' : ''}`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  {/* Fixed: Use backgroundSecondary instead of bg-gray-50 */}
                  ...
                  <div className={`${styles.backgroundSecondary} rounded-xl p-6 ${optimizationSettings.useShadowEffects ? 'shadow-md' : styles.border}`}>
                    ...
                    <div className="relative pt-1 mb-6">
                      <div className="flex mb-2 items-center justify-between">
                        <div>
                          <span className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-green-600 bg-green-200 dark:bg-green-900/40 dark:text-green-400">
                            Correct
                          </span>
                        </div>
                      ...
                      <div className="flex h-4 mb-4 overflow-hidden rounded-lg bg-gray-200 dark:bg-gray-700">
                        <div style={{ width: `${(results.correctCount / results.totalQuestions) * 100}%` }} className="bg-green-500"></div>
                      </div>
                      
                      <div className="flex mb-2 items-center justify-between">
                        <div>
                          <span className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-red-600 bg-red-200 dark:bg-red-900/40 dark:text-red-400">
                            Incorrect
                          </span>
                        </div>
                        <div className="text-right">
                      ...
                      <div className="flex h-4 overflow-hidden rounded-lg bg-gray-200 dark:bg-gray-700">
                        <div style={{ width: `${((results.totalQuestions - results.correctCount) / results.totalQuestions) * 100}%` }} className="bg-red-500"></div>
                      </div>
                    </div>
                    
                    <div className="mt-6">
                      <h4 className={`font-medium text-sm ${styles.textSecondary} mb-2`}>Areas to focus on:</h4>
                      {groupedQuestions.incorrect.length > 0 ? (
                        <ul className="space-y-1">
                          ...
                          {groupedQuestions.incorrect.length > 3 && (
                            <li className="text-sm text-purple-600 font-medium cursor-pointer hover:underline" onClick={() => handleTabChange('incorrect')}>
                              View all incorrect answers →
                            </li>
                          )}
                        </ul>
                      ) : (
                        <p className={`text-sm ${styles.textMuted}`}>Great job! You answered all questions correctly.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {groupedQuestions.incorrect.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    {groupedQuestions.incorrect.map((question, index) => (
                      <div 
                        key={index} 
                        // Fixed: Added dark mode classes for incorrect cards
                ...
                <div className="space-y-4">
                  {results.questions && results.questions.map((question, index) => {
                    ...
                    return (
                      <div 
                        key={index} 
                        // Fixed: Dynamic background based on correctness with dark mode support
                      ...
                      >
                        <div className="flex justify-between">
                          <span className={`font-medium ${styles.text}`}>Question {index + 1}</span>
                          <span className={`flex items-center ${isCorrect ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                            {isCorrect ? (
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                              </svg>
                        ...
                        <div className={`mt-4 ${!isCorrect ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : ''}`}>
                          <div className={`bg-white/60 dark:bg-gray-900/40 p-3 rounded-lg border ${isCorrect ? 'border-green-200 dark:border-green-800' : 'border-red-200 dark:border-red-800'}`}>
                            <div className={`text-sm ${styles.textMuted}`}>Your answer:</div>
          ...
          {(activeTab === 'incorrect' || activeTab === 'all') && (
            // Fixed: Background for bottom actions
