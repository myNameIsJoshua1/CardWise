...
const QuizResults = () => {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-gray-50 flex justify-center items-center" role="status" aria-live="polite">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" aria-hidden="true" />
        ...
        <div className={`bg-white rounded-xl ${optimizationSettings.useShadowEffects ? 'shadow-xl' : 'border border-gray-200'} overflow-hidden`}>
            ...
            <div className="mt-8 flex flex-col md:flex-row items-center justify-between">
              <div className="relative flex items-center">
                ...
                <div className={`ml-4 flex flex-col items-start ${optimizationSettings.useAnimations ? 'animate-fade-in' : ''}`}>
                  ...
                  <div className="text-white mt-1 flex items-center"><span className="text-2xl mr-2">{grade.emoji}</span><span>{grade.description}</span></div>
                </div>
              </div>
          ...
          <div className="px-4 border-b">
            <div className="flex overflow-x-auto -mb-px space-x-8">
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === 'overview' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              >Overview</button>

              <button
                role="tab"
                aria-selected={activeTab === 'incorrect'}
                tabIndex={0}
                onKeyDown={(e) => handleTabKey(e, 'incorrect')}
                onClick={() => handleTabChange('incorrect')}
                className={`py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === 'incorrect' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              >Incorrect ({groupedQuestions.incorrect.length})</button>
              ...
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${activeTab === 'all' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              >All Questions ({(results.questions ?? []).length})</button>
            </div>
          </div>
              ...
              <div className={`${optimizationSettings.useAnimations ? 'animate-fade-in' : ''}`}>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className="bg-green-500 h-2 rounded-full" style={{ width: `${stats.accuracy}%` }} />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${stats.percentComplete}%` }} />
                        </div>
                      </div>
                      ...
                      <div className="pt-2">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-semibold flex items-center">{grade.emoji}<span className={`ml-1 ${grade.color}`}>{grade.letter}</span></p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className={`bg-gray-50 rounded-xl p-6 ${optimizationSettings.useShadowEffects ? 'shadow-md' : 'border border-gray-200'}`}>
                    ...
                    <div className="relative pt-1 mb-6">
                      ...
                      <div className="flex h-4 mb-4 overflow-hidden rounded-lg bg-gray-200">
                        <div style={{ width: `${(results.totalQuestions ?? 0) > 0 ? ((results.correctCount ?? 0) / (results.totalQuestions ?? 0)) * 100 : 0}%` }} className="bg-green-500" />
                      </div>
                      ...
                      <div className="flex mb-2 items-center justify-between">
                        ...
                        <div className="text-right">
                          <span className="text-xs font-semibold inline-block text-red-600">{(results.totalQuestions ?? 0) - (results.correctCount ?? 0)}/{results.totalQuestions ?? 0}</span>
                        </div>
                      </div>
                      <div className="flex h-4 overflow-hidden rounded-lg bg-gray-200">
                        <div style={{ width: `${(results.totalQuestions ?? 0) > 0 ? (((results.totalQuestions ?? 0) - (results.correctCount ?? 0)) / (results.totalQuestions ?? 0)) * 100 : 0}%` }} className="bg-red-500" />
                      </div>
                    </div>
                      ...
                      {groupedQuestions.incorrect.length > 0 ? (
                        <ul className="space-y-1">
                          {groupedQuestions.incorrect.slice(0, 3).map((q, i) => (
                            <li key={i} className="text-sm text-gray-600 flex items-start"><span className="mr-2 text-red-500">•</span><span className="line-clamp-1">{q.question}</span></li>
                          ))}
                ...
                <div className="flex justify-between items-center mt-8">
                  ...
                  <div className="flex space-x-4">
                    <button onClick={handleBackToDeck} className="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">Back to Deck</button>
                    <button onClick={handleRetryQuiz} className="px-4 py-2 bg-gradient-to-r from-purple-600 to-orange-500 text-white rounded shadow-sm">Try Again</button>
                  </div>
                </div>
              </div>
            )}
                ...
                {groupedQuestions.incorrect.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    {groupedQuestions.incorrect.map((question, index) => (
                      <div key={index} className={`p-6 rounded-xl ${optimizationSettings.useShadowEffects ? 'shadow-sm' : 'border'} border-red-200 bg-red-50`}>
                        <div className="flex justify-between"><span className="font-medium">Question {index + 1}</span><span className="text-red-600 flex items-center">Incorrect</span></div>
                        <p className="mt-3 text-gray-800 font-medium">{question.question}</p>
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-white/60 p-3 rounded-lg border border-red-200"><div className="text-sm text-gray-500">Your answer:</div><div className="font-medium text-red-600">{question.userAnswer ?? '(No answer)'}</div></div>
                          <div className="bg-white/60 p-3 rounded-lg border border-green-200"><div className="text-sm text-gray-500">Correct answer:</div><div className="font-medium text-green-600">{question.correctAnswer}</div></div>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-12"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div><h3 className="text-xl font-semibold mb-2">Perfect Score!</h3><p className="text-gray-600">You answered all questions correctly. Great job!</p></div>
                )}
              </div>
            )}
          ...
          {(activeTab === 'incorrect' || activeTab === 'all') && (
            <div className="p-6 border-t bg-gray-50 flex justify-between">
              <button onClick={handleBackToDeck} className="px-4 py-2 bg-gray-200 text-gray-800 rounded">Back to Deck</button>
              <button onClick={handleRetryQuiz} className="px-4 py-2 bg-gradient-to-r from-purple-600 to-orange-500 text-white rounded">Try Again</button>
            </div>
          )}
        </div>
      </div>
